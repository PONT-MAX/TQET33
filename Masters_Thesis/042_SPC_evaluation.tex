...
\begin{minipage}[t]{0.3\linewidth} %Sit
	...
	\label{fig:sit_m30}
\end{minipage}
	\caption{Images reconstructed using subsampling ratios between 5\% to 30\%. Reference image from a starting with 5\% from top down and .} 
	\label{fig:subsampling_ratios}
\end{figure}

\begin{itemize}
\item Redan vid 5\% kan bildens scen uppfattas till skillnad mot minimum där man skulle kunna anta vad som är i bilden men många detaljer är förlorade
\item Bilderna beter sig som förväntat att kvaliten ökar med ration.
\item Mellan 10-15\% börjar de finaste detaljenra komma fram så som skivorna i husfasaden, texten på bilen blir skarpare och ansiktena på människorna.
\item Över 15\% är ökar inte bildkvaliten lika mycket, men förbättrad kvalitet i detaljerna kan ses.
\end{itemize}
